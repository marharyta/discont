...
exports.deleteDashboardItem = deleteDashboardItem;
...
const {
  detectOnlineStore,
  checkAsosItemInDB
} = require("../asosScraper/utils"); // const Worker = require('webworker-threads').Worker;
...
function getDashboardItem(req, res, next) {
  if (req.session.user) {
...
async function deleteDashboardItem(req, res, next) {
  if (req.session.user) {
    deleteItem({
      productId: req.params.itemId
    }).then(d => res.redirect("/asosItems")).catch(err => {
      next(err);
    });
  } else {
    res.redirect("/login");
...
async function addUrl(req, res, next) {
  ...
  if (storePrint === "asos") {
    await checkAsosItemInDB(url1.href, data => {
      asosDBManager.updateAsosProductInDB(data, req.session.user);
      res.redirect(`/asosItems/${data.productId}?productExists=true`);
    }, () => {
      // makeRequest(res);
      console.log('here is where we would normally make a request'); // const worker = new Worker(function () {
      ...
      scrapeAsosProductPage(url1.href, req.session.user).then(data => {
        console.log(data); // optimistic UI
